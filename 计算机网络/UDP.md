...
对于Skype for Business，TCP比UDP更精确，Skype for Business将使用TCP传输IM，表情符号，图片等，因为此数据包需要高度的数据完整性，如果有数据丢失，则IM，表情符号，我们发送的照片将无法识别，收件人无法识别它们。对于UDP，此协议始终保持数据包的传输，并且将应用于音频和视频，它可以容纳更多的数据包，并且在传输数据包时也会丢失一些数据包，但是对于音频和视频来说，这是难以察觉的数据丢失不会影响数据完整性，收件人仍然可以理解和识别它。
...
总而言之，在Skype for Business中，TCP用于IM，表情符号，需要高数据完整性的图片，而UDP用于音频和视频，它们需要高数据包但不需要高数据完整性。



- 但是在高速，低延迟的网络上，两者是相同的。



首先是它不检查数据包是否被完全接收。

另一个是该协议对数据包重新排序不敏感。这意味着UDP可以处理它，而无需将其视为数据包丢失。



但是在UDP服务器中，没有检查是否已到达包。 甚至服务器都不关心它。 它只是将数据包发送到客户端，通常在需要视频流，直播广播等高流的地方使用UDP。



另一方面，**实时视频流**选择**UDP，**因为在实时流中几乎无法进行预取-缓冲越多，视频播放延迟就越大。鉴于UDP仅提供最基本的传输层功能，因此它通常与其他应用层，特定于视频的协议（例如RTSP）一起使用以执行视频流。
...
现阶段带宽已经不是网络传输的瓶颈，往返时间才是，所以新的网络传输协议可以适当增加数据冗余，减少重传操作。
...
![截屏2020-08-13 下午2.29.01](/Users/jieyang/Library/Application Support/typora-user-images/截屏2020-08-13 下午2.29.01.png)

https://www.jianshu.com/p/bb3eeb36b479



所以 QUIC 协议选择了 UDP，因为 UDP 本身没有连接的概念，不需要三次握手，优化了连接建立的握手延迟，

![截屏2020-08-13 下午2.32.27](/Users/jieyang/Library/Application Support/typora-user-images/截屏2020-08-13 下午2.32.27.png)
...
作者：Auditore
链接：https://www.jianshu.com/p/bb3eeb36b479
来源：简书
